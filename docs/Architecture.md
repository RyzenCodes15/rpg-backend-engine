...
2. **Application Layer**: Contains Use Cases that coordinate domain objects to fulfill business requirements (e.g., `AttackTargetUseCase`, `CraftItemUseCase`).
3. **Infrastructure Layer**: Implements the adapters for databases (Spring Data JPA), messaging, and external APIs.
4. **Presentation Layer**: Exposes the REST API controllers (Spring Web) consumed by the frontend.
